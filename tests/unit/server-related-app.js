...
Echo.Tests.test("checkAppkey()", function() {
	var definition = getTestControlClass();
	var control = new definition({"target": jQuery("<div>")});

	var html = control.config.get("target").html();
...
function getTestControlClassName() {
	return "MyTestControl";
};

function getTestControlClass(name) {
	return Echo.Utils.getComponent(name || getTestControlClassName());
};

function initTestControl(config, name) {
	createTestControl("MyTestControl");
	var definition = getTestControlClass(name);
...
function createTestControl(name, config) {
	Echo.Control.create(getControlManifest(name, config));
};

function getControlManifest(name, config) {
	config = config || {};

	var manifest = Echo.Control.manifest(name || getTestControlClassName());
