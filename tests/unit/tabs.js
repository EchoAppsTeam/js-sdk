...
suite.prototype.tests.commonWorkflow = {
	...
	"check": function() {
		...
		this.jqueryObjectsEqual($(panel1.html()), $(tabsParams.entries[0].panel.html()), "Check than first panel has been added properly");
		this.jqueryObjectsEqual($(panel2.html()), $(tabsParams.entries[1].panel.html()), "Check than second panel has been added properly");

		QUnit.ok(tab1.html() === tabsParams.entries[0].label
				&& tab2.html() === tabsParams.entries[1].label, "Check that tab labels are displayed");

		QUnit.ok(tab1.hasClass("extra-class")
				&& !tab2.hasClass("extra-class"), "Check that extraClass has been added");

		QUnit.ok(tab2.hasClass("disabled")
				&& !tab1.hasClass("disabled"), "Check that disabled class has been added");
		...
		QUnit.ok(!echoTabs.get(tabsParams.entries[0].id).length
				&& echoTabs.get(tabsParams.entries[1].id).length, "Check get() method");

		QUnit.ok(!echoTabs.has(tabsParams.entries[0].id)
				&& echoTabs.has(tabsParams.entries[1].id), "Check has() method");
		...
		QUnit.ok(tab2.html() === "New label"
				...
				&& echoTabs.getPanels().find("div[id='" + tabsParams.entries[1].id  + "']").html() === "New content", "Check update() method");
