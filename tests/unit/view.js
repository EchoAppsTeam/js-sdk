...
var suite = Echo.Tests.Unit.View = function() {};
...
suite.prototype.tests.PublicInterfaceTests = {
	"check": function() {
		...
		view.render({"template": suite.templates.small});
		QUnit.ok(!view.get("container"),
		...
		var view = new Echo.View({"cssPrefix": "echo-"});
		var dom = view.render({
			"template": suite.templates.incorrect
		});
		...
		var checks = function(view, prefix) {
			...
			QUnit.equal(view.get("header").html(), "Header Renderer applied!",
				prefix + " Checking if the header renderer was applied once again after element cleanup");
		};
		...
		checks(view, "[renderers in config]");

		// forking another view
		var _view = view.fork();
		_view.render({"template": suite.templates.simple});
		QUnit.equal(view.get("header").html(), "Header Renderer applied!",
...
suite.templates.small =
	'<div class="echo-wrapper"></div>';

suite.templates.incorrect =
	'<div class="echo-container">' +
