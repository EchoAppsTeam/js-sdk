...
So, if the data parameter is empty we should fetch it from the server through the ["Echo.StreamServer.API.Request"](#!/api/Echo.StreamServer.API.Request-method-constructor) class. Definition of the _request method will be described below. Else, if the data parameter is defined, we should ["render"](#!/api/Echo.Control-method-render) control and call ["ready"](#!/api/Echo.Control-method-ready) method to detect that control is ready. All functions that defined in the manifest namespace have control context (this variable pointed to control instance) if it this is not described explicit.
...
Now we can define html template for the control UI. It should be a string value which may contains html elements (DOM structure) and placeholder. According to control initialization this value will be compiled by the ["Echo.Control.substitute"](#!/api/Echo.Control-method-substitute) method. This method compiles placeholders to the string values.
...
Important note: as you can see, the template contains the placeholder "{data:count}". This placeholder will be processed by the template engine (due to "substitute" method describes above) before thw template is inserted into control UI. You can find the general description of the rendering engine in the ["Terminlogy and dev tips" guide](#!/guide/terminology). In addition to the basic placeholders supported by the rendering engine, the base controls functionality also provides the ability to define the following placeholders:
...
- we try to get request object through the ["get"](#!/api/Echo.Control-method-get) method defined in the Echo.Control class. This object should be instantiated from Echo.StreamServer.API.Request class for request items count number. We use a ["static method request"](#!/api/Echo.StreamServer.API.Request-static-method-request) to instantiating. After request object definition we store it to the internal through the ["set"](#!/api/Echo.Control-method-set).
...
- if we get another "errorCode" we can reflect it by the type of error (critical or not) and request type. This request options defined in the ["Echo.API.Transport"](#!/api/Echo.API.Transport) class.
...
## Features which aren't described in this guide

If you look at the ["manifest"](#!/api/Echo.Control-static-method-manifest) doc you can see that we ommit some manifest fields. It is so because described fields are quite enough for Counter control.
