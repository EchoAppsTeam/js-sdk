module.exports = function(grunt) {
	...
	var helpers = require("../lib.js").init(grunt);
	...
	FtpUploader.prototype.start = function() {
		...
		this.client.auth(this.config.auth.user, this.config.auth.password, function(err) {
			...
			!helpers.env("debug") && grunt.verbose.writeln("Starting upload");
			self.currentStep = 1;
	...
	grunt.registerInitTask("release", "Release", function() {
		...
		switch (target) {
			...
			default:
				...
				var auth = config.release.auth[helpers.env("production") ? "cdn" : "sandbox"];
				new FtpUploader({
					"complete": done,
					"auth": auth,
					"uploads": uploads
	...
	grunt.registerHelper("push_pages", function(done) {
		grunt.helper("make_docs", function() {
			...
			if (helpers.env("debug") || !helpers.env("production")) {
				console.log(updateCmd);
				done();
				return;
			}
			helpers.exec(updateCmd, function() {
				grunt.log.ok();
				done();
			});
		});
