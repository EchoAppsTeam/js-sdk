module.exports = function(grunt) {
	...
	grunt.registerHelper("make_docs", function(callback) {
		shared.exec("jsduck --version | awk '{ print $2; }'", function(version) {
			var desiredVersion = "5.0.0.beta3";
			var failed = false;
			version = _.trim(version);
			if (!version) {
				failed = true;
				grunt.log.writeln("jsduck is not installed. Install it by running command `" + ("sudo gem install jsduck -v " + desiredVersion).yellow + "`.").cyan;
